...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!isset($_POST['id']) || empty($_POST['id'])) {
        // Check for duplicate assessment with same batch, module, and main component
        $check_query = "SELECT COUNT(*) as count FROM assessments WHERE batch_id = ? AND module_id = ? AND main_component_id = ?";
        $check_stmt = $conn->prepare($check_query);
        $check_stmt->bind_param("iii", $batch_id, $module_id, $main_component_id);
        $check_stmt->execute();
        ...
        if ($row['count'] > 0) {
            ...
            echo "<script>alert('Assessment already exists for this Batch, Module, and Main Component combination.'); window.location.href='../exams.php?error=1';</script>";
            exit();
        }
    }
    ...
    for ($i = 1; $i <= 4; $i++) {
        ...
        if (isset($_POST[$remove_key]) && $_POST[$remove_key] == '1') {
            ...
            if (isset($_POST['id']) && !empty($_POST['id'])) {
                $query = "SELECT $attachment_key FROM assessments WHERE id = ?";
                ...
                if (!empty($existing_attachment)) {
                    $file_path = "../uploads_exam_assessments/" . $existing_attachment;
    ...
    // Execute the query
    if ($stmt->execute()) {
        // Redirect back to the exams page with a success message
        $_SESSION['message'] = "Assessment added/updated successfully.";
        header("Location: ../exams.php?success=1");
        exit();
    } else {

        // Redirect back to the exams page with an error message
        header("Location: ../exams.php?error=1&msg=" . urlencode($stmt->error));
        exit();
    }
