...
// JSON endpoint safety — NEVER let PHP warnings/errors leak into output (breaks JSON.parse)
// - display_errors = 0: hide HTML warnings from response (they still go to server error log)
// - output buffer + ob_clean() before echo: wipe any accidental output
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (strpos($programme_name, 'Bachelor of Business Management') !== false || $programme_id == 47) { // 47 is the BBM program ID
            ...
            $studentsQuery = "SELECT s.student_code, s.first_name, s.last_name, ap.student_registration_id, ap.compulsory_sub, ap.elective_subs 
                              FROM students s 
                              JOIN allocate_programme ap ON s.student_code = ap.student_code 
                              WHERE ap.programme_code = ? AND ap.batch_id = ? AND LOWER(ap.status) = 'active' ";

            $bbmStudentTypes = '';
            $bbmStudentParams = [$programme_id, $batch_id];
            $bbmStudentParamTypes = 'ii';

            if (!empty($student_status)) {
                $studentsQuery .= " AND ap.status = ? ";
                $bbmStudentParams[] = $student_status;
                $bbmStudentParamTypes .= 's';
            }

            $studentsStmt = $conn->prepare($studentsQuery);
            $studentsStmt->bind_param($bbmStudentParamTypes, ...$bbmStudentParams);
            $studentsStmt->execute();
            $studentsResult = $studentsStmt->get_result();

            $students = [];
            while ($row = $studentsResult->fetch_assoc()) {
                // Check if module is in compulsory_sub or elective_subs
                // Fix PHP 8.x Deprecated: explode(null) → cast NULL to ''
                $compulsory_subs = explode(',', (string)($row['compulsory_sub'] ?? ''));
                $elective_subs_array = explode(',', (string)($row['elective_subs'] ?? ''));
            ...
            // Add the detailed results to the student data
            foreach ($students as $studentId => $studentData) {
                // Add questions data to the student results
                if (isset($detailedResults[$studentId])) {
            ...
            while (ob_get_level()) {
                ob_end_clean();
            } // wipe any leaked warnings/output
            ...
            // First get all students with their compulsory_sub and elective_subs
            $studentsQuery = "SELECT ap.student_code, ap.compulsory_sub, ap.elective_subs 
                              FROM allocate_programme ap
                              WHERE ap.programme_code = ? AND ap.batch_id = ? AND LOWER(ap.status) = 'active' ";

            $allowedParams = [$programme_id, $batch_id];
            $allowedTypes = 'ii';

            if (!empty($student_status)) {
                $studentsQuery .= " AND ap.status = ? ";
                $allowedParams[] = $student_status;
                $allowedTypes .= 's';
            }

            $studentsStmt = $conn->prepare($studentsQuery);
            $studentsStmt->bind_param($allowedTypes, ...$allowedParams);
            $studentsStmt->execute();
            $studentsResult = $studentsStmt->get_result();
            ...
            while ($row = $studentsResult->fetch_assoc()) {
                ...
                if (in_array($module_name, $compulsory_subs) || in_array($module_name, $elective_subs_array)) {
                    $allowed_student_ids[] = $row['student_code'];
                }
            }

            // Define programme type first
            if (strpos($programme_name, 'Executive Certificate in Management') !== false) {
                $programme_type = 'ECM';
            } else if (strpos($programme_name, 'Graduate Diploma in Management (Level 6)') !== false) {
                $programme_type = 'GDM';
            } else if (strpos($programme_name, 'BTEC Higher National Diploma in Business') !== false) {
                $programme_type = 'BTEC';
            } else if (
                strpos($programme_name, 'BSc (Hons) in Software Engineering') !== false ||
            ...
            // Initialize componentResits array (needed before early returns / re-use below)
            $componentResits = [];
            foreach ($components as $component) {
            ...
            if (empty($allowed_student_ids)) {
                $response = [
                    ...
                    'debug_info' => ['programme_name' => $programme_name, 'student_status' => $student_status]
                ];
                while (ob_get_level()) {
                    ob_end_clean();
                } // wipe any leaked warnings/output
            ...
            $allowed_ids_placeholders = implode(',', array_fill(0, count($allowed_student_ids), '?'));

            $query = "SELECT 
                        sr.student_id,
                ap.student_code as student_id,
            ...
            LEFT JOIN student_results sr ON (sr.student_id = ap.student_code 
                AND sr.program_id = ? 
                AND sr.batch_id = ? 
                AND sr.module_id = ?)
            LEFT JOIN assignment_components mc ON sr.main_component_id = mc.id
            WHERE ap.programme_code = ? 
                AND ap.batch_id = ? 
                AND LOWER(ap.status) = 'active'
            ";
            ...
            $query .= " AND ap.student_code IN ($allowed_ids_placeholders)
            ";



            if ($main_component_id) {
                $query .= " AND (sr.main_component_id = ? OR sr.main_component_id IS NULL)";
            }
            if ($sub_component_id) {
                $query .= " AND (sr.sub_component_id = ? OR sr.sub_component_id IS NULL)";
            }
            $query .= " ORDER BY ap.student_code, sr.main_component_id";


            // ---------------
            // Build $params AND $types IN LOCKSTEP below so their length is GUARANTEED to match.
            // This eliminates the ArgumentCountError on bind_param.
            // ---------------

            $params = [];
            $types  = '';

            // LEFT JOIN student_results placeholders: program_id, batch_id, module_id
            $params[] = $programme_id;
            $types .= 'i';
            $params[] = $batch_id;
            $types .= 'i';
            ...
            // IN (...) allowed student IDs
            foreach ($allowed_student_ids as $sid) {
                $params[] = $sid;
                $types .= 'i';
            }

            // Optional: main component filter
            if ($main_component_id) {
                $params[] = $main_component_id;
                $types .= 'i';
            }

            // Optional: sub component filter
            if ($sub_component_id) {
                $params[] = $sub_component_id;
                $types .= 'i';
            }
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                if (!isset($studentResults[$studentId])) {
                    $studentResults[$studentId] = [
                        ...
                        'student_registration_id' => $row['student_registration_id'],
                        'student_name (student_registration_id)' => $row['student_name'] . ' (' . $row['student_registration_id'] . ')',
                        'components' => []
                    ];
                }
                ...
                if (strpos($programme_name, 'Executive Certificate in Management') !== false) {
                    ...
                    // Loop through all ECM resits dynamically
                    for ($i = 1; $i <= 4; $i++) {
                        $markKey = "ecm_r{$i}_converted_marks";
                        $gradeKey = "ecm_r{$i}_grade"; // assuming you have grades for ECM resits

                        if (isset($row[$markKey]) && $row[$markKey] !== null && $row[$markKey] !== '') {
                            $resultData["resit_$i"] = $row[$markKey];

                            // Include grade if exists
                            if (isset($row[$gradeKey]) && $row[$gradeKey] !== null && $row[$gradeKey] !== '') {
                                $resultData["resit{$i}_Grade"] = $row[$gradeKey];
                            }
                ...
                //         'main_result_Grade' => $row['hd_grade'],
                //         'component_type' => $row['component_type'],
                //         'component_name' => $row['component_name']
                //     ];
                //     for ($i = 1; $i <= 7; $i++) {
                ...
                //             // Correctly add the resit value and grade to the result data (don't overwrite other data)
                ...
                // neew on 27.02.2026 added this code 
                else if (
                    strpos($programme_name, 'BSc (Hons) in Software Engineering') !== false ||
                    strpos($programme_name, 'Higher Diploma in Biomedical Science') !== false ||
                    strpos($programme_name, 'Higher Diploma in Biotechnology') !== false ||
                    ...
                    // Handle main_result conversion for special values
                    $main_result_val = $row['hd_converted_marks'];
                    if ($main_result_val === null || $main_result_val === '') {
                        $main_result_display = '';
                    } elseif ($main_result_val == -1) {
                        $main_result_display = 'Not Submitted';
                    } elseif ($main_result_val == -2) {
                        $main_result_display = 'Absent';
                    } else {
                        $main_result_display = $main_result_val;
                    }

                    // Initialize the resultData
                    $resultData = [
                        'main_result' => $main_result_display,
                        'main_result_Grade' => $row['hd_grade'],
                        'component_type' => $row['component_type'],
                        'component_name' => $row['component_name']
                    ];
                    ...
                    for ($i = 1; $i <= 7; $i++) {
                        ...
                        // Only show if the resit value key is set (it may be 0 or -1)
                        if (isset($row[$key]) && $row[$key] !== null && $row[$key] !== '') {
                            $resit_val = $row[$key];
                            if ($resit_val == -1) {
                                $resit_display = 'Not Submitted';
                            } elseif ($resit_val == -2) {
                                $resit_display = 'Absent';
                            } else {
                                $resit_display = $resit_val;
                            }

                            // Mark that the resit data exists for this component
                            $componentResits[$componentId]["resit_$i"] = true;

                            $resultData["resit_$i"] = $resit_display;
                            $resultData["resit{$i}_Grade"] = $row[$gradeKey];
                        }
                    }
            ...
            // Programme type already defined earlier
