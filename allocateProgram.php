...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $student_registration_id = isset($_POST['student_registration_id']) ? $_POST['student_registration_id'] : null;
    $compulsory_subs = $_POST['compulsory_subs'];
    $elective_subs = $_POST['elective_subs'];

    // Check for duplicate student_registration_id
    // $checkSql = "SELECT COUNT(*) FROM `allocate_programme` WHERE student_registration_id = ? OR student_code= ? ";
    // $checkStmt = $conn->prepare($checkSql);
    // $checkStmt->bind_param("ss", $student_registration_id, $student_code);
    // $checkStmt->execute();
    ...
    // Check for duplicate allocation (student_code + programme_code + batch_id)
    $checkSql = "SELECT COUNT(*) FROM allocate_programme 
    WHERE student_code = ? 
    AND programme_code = ? 
    AND batch_id = ?";

$checkStmt = $conn->prepare($checkSql);
$checkStmt->bind_param("iii", $student_code, $programme_code, $batch_id);
$checkStmt->execute();
$checkStmt->bind_result($count);
$checkStmt->fetch();
$checkStmt->close();
    ...
    if ($count > 0) {
        echo '<script>alert("Duplicate Student Registration ID found! Or Re Entered Same Student");</script>';
    } else {
        // Insert data into `allocate_programme` table

        $sql = "INSERT INTO `allocate_programme` (student_code, university_id, programme_code, batch_id, student_registration_id, elective_subs, compulsory_sub,entered_by) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)";

        $stmt = $conn->prepare($sql);
        $stmt->bind_param("iiiissss", $student_code, $university_id, $programme_code, $batch_id, $student_registration_id, $elective_subs, $compulsory_subs, $Session_username);


        if ($stmt->execute()) {
            $_SESSION['message'] = "Program allocated successfully!";
            echo '<script>
        ...
        <div id="content">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <div class="form-group">
                                                <div class="row">
                                                    ...
                                                    <div class="col-md-8"><input type="text" placeholder="Student Registration ID" id="registration_id" name="student_registration_id" class="form-control"></div>
                                                </div>
                                            </div>
                                        </div>
            ...
            <script>
                ...
                function formatBatchLabel(batch) {
                    const status = (batch.batch_hide_active || '').toLowerCase();
                    if (!isSuperAdmin || status === '') {
                        return batch.batch_name;
                    }
                    if (status === 'active') {
                        return batch.batch_name + ' (Active)';
                    }
                    if (status === 'inactive') {
                        return batch.batch_name + ' (Inactive)';
                    }
                    return batch.batch_name;
                }

                function populateBatchOptions(batchData, selectedBatchId) {
                    $('#batch').empty().append('<option value="">Select Batch</option>');
                    $.each(batchData, function(key, value) {
                        $('#batch').append(
                            `<option value="${value.id}">${formatBatchLabel(value)}</option>`
                        );
                    });
                    if (selectedBatchId) {
                ...
                function loadModules(programmeCode) {
                                    ...
                                    if (moduleType === 'Compulsory') {
                                        moduleHTML = `
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox" id="compulsoryModule${moduleId}" name="compulsory_modules[]" value="${moduleName}" checked>
                ...
                $(document).ready(function() {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            $.each(data, function(key, value) {
                                $('#student').append(`<option value="${value.student_code}">${value.first_name} ${value.last_name} (NIC: ${value.nic})</option>`);
                            });
                        }
                    });
                        ...
                        if (programmeCode) {
                            $.ajax({
                                ...
                                success: function(batchData) {
                                    populateBatchOptions(batchData, selectedBatchId);
                                }
                            });
                        }
                    }
