        ...
        $(document).ready(function() {
            $('#studentsTable').DataTable({
                ...
                "columnDefs": [{
                        "targets": 14,
                        "visible": false
                    } // hide Active Students column (index 14), but keep its data for grouping
                ],
                rowGroup: {
                    dataSrc: function(row) {
                        // Combine Program Name (col 3) + Batch Name (col 4) + Active Count (col 14) into one group label
                        return row[3] + ' - ' + row[4] + ' (Active Students: ' + row[14] + ')';
                    }
                }
            });
        });
