...
<script>
    $(document).ready(function() {
        ...
        $('#confirmPaymentBtn').on('click', function() {
            const regIDValue = document.getElementById('specific_student_reg_id').value; // This should already be set
            const NewRegIDValue = document.getElementById('new_reg_id').value; // This should already be set

            // Use window.programme_code to ensure it's defined
            $.ajax({
                url: 'your_php_handler.php', // Replace with your actual PHP file
                method: 'POST',
                data: {
                    reg_id: regIDValue,
                    new_reg_id: NewRegIDValue,
                    programme_code: window.programme_code, // Use the global variable
                    batch_id: window.batch_id, // Use the global variable
                    student_id: window.student_id // This should be the student ID
                },
                success: function(response) {
            ...
            $('#rcp_student_regID').text(regIDValue); // Ensure the receipt shows the correct registration ID

            var paidDate = $('#modalPaidDate').val();

            var receiptNumber = Math.floor(Math.random() * 900000) + 100000; // Generates a 6-digit number
                ...
                success: function(response) {
                    ...
                    try {
                        ...
                        if (result.success) {
                                ...
                                success: function(dueResponse) {
                                    try {
                                        ...
                                        if (dueResult.success) {
                                            ...
                                            console.log('Outstanding totals:', dueResult.totals);
                                        } else {
                            ...
                            // $('#rcp_student_regID').text(studentregID);
                            ...
                            if (paymentMethod === 'card') {
                                // Show only the Bank Name and hide Deposit Date for card payment
                                $('#rcp_payment_bank').parent().show(); // Show the Bank Name section
                                $('#rcp_payment_dep_date').parent().hide(); // Hide the Deposit Date section

                                // Capture the selected bank name for card payment
                                selectedBank = $('#bankName').val(); // This will capture the selected bank

                                // Update the UI with the bank name or default text if not selected
                                ...
                                // Show both Bank Name and Deposit Date for bank deposit payment
                                $('#rcp_payment_bank').parent().show(); // Show the Bank Name section
                                $('#rcp_payment_dep_date').parent().show(); // Show the Deposit Date section

                                // Capture the selected bank name for bank deposit payment
                                selectedBank = $('#bankName').val(); // This will capture the selected bank

                                // Capture the selected deposit date
                                selectedDate = $('#paymentDate').val(); // This will capture the selected deposit date

                                // Update the UI with the bank name or default text if not selected
                                $('#rcp_payment_bank').text(selectedBank ? selectedBank : 'No bank selected');

                                // Update the UI with the deposit date or default text if not selected
                                ...
                                // Hide both Bank Name and Deposit Date for cash payment
                                $('#rcp_payment_bank').parent().hide(); // Hide the Bank Name section
                                $('#rcp_payment_dep_date').parent().hide(); // Hide the Deposit Date section
                            ...
                            if ($('#pay_checkbox').is(':checked')) {
                                ...
                                if (paymentAmount > 0) {
                                    let currencyType = $('#madal-crc-Types').text().trim(); // Get the text inside the element
                                    $('#receipt-details').append(`<tr><td class="tcolor">Uni Payment </td><td>${paymentAmount} <span style="color:red; font-weight:bold">[${currencyType}]</span> </td></tr>`);
                                }
                                ...
                                if (paymentAmountLKR > 0) {
                                    ...
                                    totalPayment += paymentAmountLKR;
                                }
                            }

                            // Final Total Payment
                            $('#receipt-details').append(`<tr><td class="tcolor"><strong>Total</strong></td><td><strong>${totalPayment.toFixed(2)}</strong></td></tr>`);
                            ...
                            html2pdf()
                                ...
                                .get('pdf')
                                .then(function(pdf) {
                                    // Convert PDF to Blob and send it to the server
                                    var pdfData = pdf.output('blob');

                                    // Create FormData to upload the PDF
                                    var formData = new FormData();
                                    formData.append('pdf', pdfData, 'receipt.pdf'); // Send the generated PDF
                                    ...
                                    // Send the FormData (PDF and additional data) to the server using AJAX
                                    var xhr = new XMLHttpRequest();
                                    // xhr.open('POST', 'upload_pdf.php', true); // PHP script that will handle the upload 
                                    xhr.open('POST', 'upload_pdfs.php', true); // PHP script that will handle the upload 
                                    xhr.onload = function() {
                                        if (xhr.status === 200) {
