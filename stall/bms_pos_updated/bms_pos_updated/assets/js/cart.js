$(function () {
  ...
  //   var userWantsReceipt = false;
  //   var preReceiptWindow = null;

  //   if (isMobile) {
  //     userWantsReceipt = confirm(
  //       "Do you want to print the receipt after purchase?"
  //     );
  //     if (userWantsReceipt) {
  //       preReceiptWindow = window.open("about:blank", "_blank");
  //       if (!preReceiptWindow) {
  ...
  //         // Mobile printing
  //         if (isMobile && userWantsReceipt && preReceiptWindow) {
  //           preReceiptWindow.location.href = receiptUrl;

  //           preReceiptWindow.onload = function () {
  //             try {
  //               preReceiptWindow.focus();
  //               preReceiptWindow.print();

  //               // After print, close preview tab & redirect main page
  //               preReceiptWindow.onafterprint = function () {
  //                 preReceiptWindow.close();
  //                 window.location.href = "index.php";
  //               };

  //               // Fallback in case onafterprint is not supported
  //               setTimeout(function () {
  //                 if (!preReceiptWindow.closed) preReceiptWindow.close();
  //                 window.location.href = "index.php";
  //               }, 2000);
  //             } catch (err) {
  //               alert(
  //                 "Receipt opened — please tap your browser’s print option."
  //               );
  //               window.location.href = "index.php";
  //             }
  //           };
  ...
  //     })
  //     .fail(function () {
  //       showTempMessage("Purchase failed - please try again", "danger");
  //       $("#checkout-btn").prop("disabled", false).text("Complete Purchase");
  //     });
  // });
      ...
      .done(function (res) {
        if (res && res.success) {
          ...
          setTimeout(function () {
            if (
              confirm(
                "✅ Order placed successfully!\n\nWould you like to preview and print the receipt?"
              )
            ) {
              if (isMobile) {
                // Mobile: Open receipt in new tab with print instructions
                ...
                if (receiptWindow) {
                  ...
                  // Pop-up blocked
                  alert(
                    "⚠️ Please allow pop-ups to view the receipt.\n\nYou can view your order history from the menu."
                  );
                  window.location.href = "index.php";
                }
              } else {
                // Desktop: Open receipt in popup window
                var receiptWindow = window.open(
                  receiptUrl,
                  "receipt",
                ...
                if (receiptWindow) {
                  ...
                  alert("⚠️ Please allow pop-ups to view the receipt.");
                  window.location.href = "index.php";
                }
              }
            } else {
