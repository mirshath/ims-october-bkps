...
if (isset($_GET['action']) && $_GET['action'] == 'getReceipts' && isset($_GET['studentId'])) {
    ...
    // Fetch additional payments for this student
    $query = "SELECT afp.id, afp.total_amount, afp.paid_date, 
              GROUP_CONCAT(afi.description SEPARATOR ', ') as items_description
              FROM additional_fee_payments afp
              LEFT JOIN additional_fee_items afi ON afp.id = afi.payment_id
              WHERE afp.student_id = ? AND afp.status = 'paid'
              GROUP BY afp.id
              ORDER BY afp.paid_date DESC";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Update additional payment status to cancelled
    $updateQuery = "UPDATE additional_fee_payments SET status = 'cancelled' WHERE id = ?";
    ...
    if ($success) {
        ...
        if ($hasPaymentTypeColumn) {
            $logQuery = "INSERT INTO payment_cancellation_log (payment_id, student_id, reason, cancelled_by, cancelled_at, payment_type) VALUES (?, ?, ?, ?, NOW(), 'additional')";
        ...
        echo '<script>
            alert("Additional payment cancelled successfully!");
            ...
            <div class="container-fluid py-4">
                ...
                <div class="row mb-4">
                    <div class="col-12">
                        ...
                        <h4 class="border-bottom pb-2 mb-4">Cancel Additional Payment</h4>
                    </div>
                </div>
                        ...
                        <div class="card shadow-sm">
                            <div class="card-header bg-light">
                                <h5 class="card-title mb-0">
                                    <i class="bi bi-x-circle text-danger me-2"></i>Additional Payment Cancellation Form
                                </h5>
                            </div>
                            <div class="card-body">
                                <form id="paymentCancellationForm" method="post">
                                    ...
                                    <div class="mb-3">
                                        <label for="paymentReceiptSelect" class="form-label">Select Additional Payment:</label>
                                        <select class="form-select" id="paymentReceiptSelect" name="receiptId" required disabled>
                                            <option value="" selected disabled>Choose an additional payment...</option>
                                        </select>
                                        <div class="form-text text-muted">Select the additional payment you want to cancel</div>
                                    </div>
                                    ...
                                    <div id="receiptDetailsCard" class="card mb-3 d-none">
                                        <div class="card-body">
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-md-6">
                                                    <p class="mb-1"><strong>Items:</strong> <span id="receiptItems"></span></p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#paymentReceiptSelect').select2({
            placeholder: "Choose an additional payment...",
            width: '100%',
            theme: "classic",
            allowClear: true,
            disabled: true
        ...
        $('#studentSelect').on('select2:select', function() {
            ...
            if (studentId) {
                ...
                // Fetch additional payments for this student
                $.ajax({
                    url: 'AdditionalPaymentCancellation.php?action=getReceipts&studentId=' + studentId,
                    type: 'GET',
                    dataType: 'json',
                    success: function(data) {
                        // Clear and reset the payment receipt dropdown
                        $('#paymentReceiptSelect').empty();
                        $('#paymentReceiptSelect').append('<option value="" selected disabled>Choose an additional payment...</option>');
                        ...
                        if (data && data.length > 0) {
                            $.each(data, function(index, receipt) {
                                ...
                                const receiptText = `Rs.${receipt.total_amount || '0'} - ${formattedDate}`;
                                ...
                                $('#paymentReceiptSelect').append(
                                    ...
                                    .data('items', receipt.items_description)
                                );
                            });
                            ...
                            $('#paymentReceiptSelect').append('<option value="" disabled>No additional payments found</option>');
                        }
                    ...
                    error: function(xhr, status, error) {
                        ...
                        alert('Error fetching additional payments. Please check the console for details.');
                    }
                });
            } else {
        ...
        $('#paymentReceiptSelect').on('select2:select', function() {
            ...
            if (selectedOption.val()) {
                ...
                $('#receiptItems').text(selectedOption.data('items') || 'No items specified');
        ...
        $('#paymentCancellationForm').submit(function(e) {
            if (!confirm('Are you sure you want to cancel this additional payment? This action cannot be undone.')) {
                e.preventDefault();
            }
        });
    });
