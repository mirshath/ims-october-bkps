...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $from_registration_code  = $_POST['from_registration_code'];  // New BMS student ID after Swap
    $from_registration_code_old = isset($_POST['from_registration_code_old']) ? $_POST['from_registration_code_old'] : null; // this is OLD BMS student ID Before Swap

    // ------------------------------------------------------------------------------------------------------ 
    // Get values for old program and batch codes from hidden inputs
    $program_old_code = isset($_POST['Program_old_code']) ? $_POST['Program_old_code'] : '';
    $batch_old_code = isset($_POST['batch_old_code']) ? $_POST['batch_old_code'] : '';

    // ------------------------------------------------------------------------------------------------------ 

    $bms_registration_code   = isset($_POST['bms_registration_code']) ? $_POST['bms_registration_code'] : null; // new registration BMS Auto gen 
    // $university_id           = $_POST['university_id'];
    $university_id           = isset($_POST['university_id']) ? $_POST['university_id'] : 1;
    ...
    if ($allocateStmt = $conn->prepare($allocateQuery)) {
        ...
        if ($allocateStmt->execute()) {
            ...
            $query = "INSERT INTO batch_swap_table 
                (student_id, from_student, from_university, from_programme, from_batch, 
                 from_registration_code, university_id, programme_code, batch_id, allocated_id, entered_by, OLD_registration_id) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)";
            if ($stmt = $conn->prepare($query)) {
                $stmt->bind_param(
                    ...
                    $allocated_id,
                    $Session_username,
                    $from_registration_code_old
                );
                if ($stmt->execute()) {
                    // Update students table: set transfer_status = 1
                    ...
                    // After successful update, set student_registration_id = NULL in allocate_programme for old allocation
                    $nullRegQuery = "UPDATE allocate_programme SET student_registration_id = NULL WHERE student_code = ? AND programme_code = ? AND batch_id = ?";
                    if ($nullRegStmt = $conn->prepare($nullRegQuery)) {
                        $nullRegStmt->bind_param("sii", $student_id, $program_old_code, $batch_old_code);
                        $nullRegStmt->execute();
                        $nullRegStmt->close();
                    }

                    echo "<script>
                        alert('Student Batch SWAP , Allocation & Payment Plan have been successfully recorded!');
...
<div id="wrapper">
        ...
        <div id="content">
            ...
            <div class="p-3">
                <div class="d-sm-flex align-items-center justify-content-between mb-4">
                    <h4 class="h4 mb-0 text-gray-800">Student Batch SWAP</h4>
                </div>

                <!-- Filter Form -->
                <form action="batchSwap.php" method="POST">
                    <div class="row mb-5">
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="student_id" id="select_student" class="form-control select2" required>
                                ...
                                <?php
                                // Join students and allocate_programme on student_code, only where status is 'batchSwap'
                                $query = "
                                    SELECT s.*, s.student_code, s.nic, s.first_name, s.last_name
                                    FROM students s
                                    INNER JOIN allocate_programme a ON a.student_code = s.student_code
                                    WHERE a.status = 'batchSwap' AND s.transfer_status = 0
                                    ";
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="card">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="form-group">
                                        <div class="row">
                                            <div class="col-md-4">
                                                <label for="from_registration_code">Registration Code OLD</label>
                                            </div>
                                            <div class="col-md-8">
                                                <input type="text" readonly id="from_registration_code_old" name="from_registration_code_old" class="form-control" placeholder="New Registration Number" style="border: 2px solid red;">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <div class="row">
                                            <div class="col-md-4">
                                                <label for="from_registration_code">Current Registration Last Code </label>
                                            </div>
                                            <div class="col-md-8">
                                                <input type="text" readonly id="current_registration_last_code" name="current_registration_last_code" class="form-control" placeholder="New Registration Number" style="border: 2px solid red;">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="form-group fw-bolder">
                                        <div class="row">
                                            <div class="col-md-4">
                                                <label for="from_registration_code">Registration Code New For Selected Batch</label>
                                            </div>
                                            <div class="col-md-8">
                                    ...
                                    <div class="form-group row">
                                        <label for="bms_registration_have_code" class="col-md-4 col-form-label">HAVE BMS Registration ID (AG)</label>
                                        <div class="col-md-8">
                                            <input type="text"
                                                id="bms_registration_have_code"
                                                name="bms_registration_have_code"
                                                class="form-control"
                                    ...
                                    <div class="form-group row">
                                        <label for="bms_registration_code_last" class="col-md-4 col-form-label">LAST BMS Registration ID (AG)</label>
                                        <div class="col-md-8">
                                            <input type="text"
                                                id="bms_registration_code_last"
                                                name="bms_registration_code_last"
                                                class="form-control"
                                                placeholder="Auto Generated Registration Number"
                                                readonly
                                                style="border: 2px solid red;">
                                ...
                                <input type="hidden" id="Program_old_code" name="Program_old_code" value="">
                                <input type="hidden" id="batch_old_code" name="batch_old_code" value="">
                            ...
                            <div class="mt-4 text-right">
                                <button type="submit" class="btn btn-primary" id="transfer_sbt_btn">Submit Swap</button>
                            </div>
                        </div>
                    </div>
                </form>
    ...
    <script>
        $(document).ready(function() {
            ...
            //                 console.log("----- Ordered Student Registration IDs and NEW Student Registration IDs -----");

            //                 if (response.length > 0) {
            //                     // Sort response array by student_registration_id for the old system
            //                     const sortedOldResponse = response.slice().sort(function(a, b) {
            //                         const aNum = parseInt(a.student_registration_id, 10);
            //                         const bNum = parseInt(b.student_registration_id, 10);
            //                         if (!isNaN(aNum) && !isNaN(bNum)) {
            //                             return aNum - bNum;
            //                         }
            //                         if (a.student_registration_id < b.student_registration_id) return -1;
            //                         if (a.student_registration_id > b.student_registration_id) return 1;
            //                         return 0;
            //                     });

            //                     // Sort response array by new_student_registration_id for the new BMS system
            //                     const sortedNewResponse = response
            //                         .filter(item => item.new_student_registration_id && item.new_student_registration_id !== '')
            //                         .slice()
            //                         .sort(function(a, b) {
            //                             const aNum = parseInt(a.new_student_registration_id, 10);
            //                             const bNum = parseInt(b.new_student_registration_id, 10);
            //                             if (!isNaN(aNum) && !isNaN(bNum)) {
            //                                 return aNum - bNum;
            //                             }
            //                             if (a.new_student_registration_id < b.new_student_registration_id) return -1;
            //                             if (a.new_student_registration_id > b.new_student_registration_id) return 1;
            //                             return 0;
            //                         });

            //                     sortedOldResponse.forEach(function(item, index) {
            //                         console.log(
            //                             (index + 1) +
            //                             ". student_registration_id: " + item.student_registration_id +
            //                             " | new_student_registration_id: " + (item.new_student_registration_id || '')
            //                         );
            //                     });

            //                     // Get the last (highest/sorted) code for old system
            //                     const lastOldItem = sortedOldResponse[sortedOldResponse.length - 1] || {};
            //                     const lastRegId = lastOldItem.student_registration_id || '';
            //                     $('#current_registration_last_code').val(lastRegId).prop('readonly', true);

            //                     // Calculate the next registration id (old system, for from_registration_code)
            //                     let nextRegId = '';
            //                     if (!isNaN(lastRegId)) {
            //                         nextRegId = parseInt(lastRegId, 10) + 1;
            //                     } else if (/^\d+$/.test(lastRegId)) {
            //                         nextRegId = Number(lastRegId) + 1;
            //                     } else {
            //                         let match = lastRegId.match(/^(.*?)(\d+)$/);
            //                         if (match) {
            ...
            //                     // Get the last (sorted) BMS new registration code and show in #bms_registration_code_last
            //                     let lastNewRegId = '';
            //                     if (sortedNewResponse.length > 0) {
            //                         lastNewRegId = sortedNewResponse[sortedNewResponse.length - 1].new_student_registration_id;
            //                     }
            //                     $('#bms_registration_code_last').val(lastNewRegId).prop('readonly', true);

            //                     // Get value of #bms_registration_have_code
            //                     const bmsHaveCode = $('#bms_registration_have_code').val();

            //                     // Logic: if #bms_registration_have_code is empty or null, suggest (#bms_registration_code_last + 1)
            //                     let bmsSuggestedCode = '';
            //                     if (bmsHaveCode === "" || bmsHaveCode === null || typeof bmsHaveCode === "undefined") {
            //                         // Try to increment lastNewRegId (#bms_registration_code_last)
            //                         if (lastNewRegId !== '') {
            ...
            //                     } else {
            //                         // If a code exists in #bms_registration_have_code, display that one
            ...
            //                 // (no longer a flat array of objects)

            //                 // Get arrays for old and new registration ids
            //                 const oldRegIds = Array.isArray(response.student_registration_id) ? response.student_registration_id : [];
            //                 const newRegIds = Array.isArray(response.new_student_registration_id) ? response.new_student_registration_id : [];
            ...
            //                 let cleanedNewIds = newRegIds.filter(item => item !== '' && item !== null && typeof item !== "undefined");
            //                 let sortedNew = cleanedNewIds.slice().sort(function(a, b) {
            //                     const aNum = parseInt(a, 10);
            //                     const bNum = parseInt(b, 10);
            //                     if (!isNaN(aNum) && !isNaN(bNum)) return aNum - bNum;
            //                     if (a < b) return -1;
                ...
                if (batchId && programId) {
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            const newRegIds = Array.isArray(response.new_student_registration_id) ?
                            ...
                            // Optional: log the full, ordered new_student_registration_id list for debugging
                            console.log("Ordered new_student_registration_id list:", sortedNew);

                            // Last id in ordered list:
                            let lastNewRegId = sortedNew.length > 0 ? sortedNew[sortedNew.length - 1] : '';
                            $('#bms_registration_code_last').val(lastNewRegId).prop('readonly', true);
                            ...
                            // For logging/debug
                            console.log("Old student_registration_id list (filtered):", oldRegIds);
                            console.log("Original new_student_registration_id list (filtered):", newRegIds);
                            // Above: ordered list (sortedNew) is already logged

                            console.log("Last old registration:", lastOldRegId, " Next:", nextRegId);
                            console.log("Last new reg (ordered):", lastNewRegId, " Suggest:", bmsSuggestedCode);
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            if (!response.error) {
                                ...
                                $('#from_registration_code_old').val(response.student_registration_id).prop('readonly', true);
                                // $('#current_registration_last_code').val(response.student_registration_id).prop('readonly', true);
                                $('#bms_registration_have_code').val(response.new_student_registration_id).prop('readonly', false);

                                // If have code is present, also put it into #bms_registration_code
                                if (response.new_student_registration_id !== "" && response.new_student_registration_id !== null && typeof response.new_student_registration_id !== "undefined") {
                                    $('#bms_registration_code').val(response.new_student_registration_id).prop('readonly', true);
                                }

                                console.log('Programme ID OLD:', response.programme_code);
                                // Set the Program_old_code input value with the previous programme code (old allocation)
                                ...
                                $('#from_registration_code_old').css({
                                ...
                                $('#from_student, #from_university, #from_programme, #from_batch, #from_registration_code_old,#bms_registration_code').val('');
