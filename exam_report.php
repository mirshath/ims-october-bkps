...
// Define the HD programs that need special final grade calculation
$hd_programs = [
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($report_type === 'assessment' && !empty($module_id)) {
        // Get students and their results for this module with status filter
        $status_where = "";
        $bind_types = "iiiii";
        $bind_params = [$programme_id, $batch_id, $module_id, $programme_id, $batch_id];
        ...
        $students_query = "SELECT s.student_code, s.first_name, s.last_name, ap.student_registration_id, ap.*,
                          fsr.final_result 
                          FROM students s 
        ...
        // Fetch component count once if it's an HD program
        $componentCount = 0;
        if (in_array($programme_name, $hd_programs)) {
            $compCountQuery = "SELECT COUNT(*) as count FROM allocated_components WHERE module_id = ?";
            $stCC = $conn->prepare($compCountQuery);
            $stCC->bind_param("i", $module_id);
            $stCC->execute();
            $resCC = $stCC->get_result();
            if ($rowCC = $resCC->fetch_assoc()) {
                $componentCount = $rowCC['count'];
            }
        }
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $final_hd_grade = 'N/A';
            $component_results_display = [];

            // Generic component fetch for ALL programs
            $componentsQuery = "
                SELECT ac.as_main_component_name, sc.sub_component_name, 
                       sr.*
                ...
                WHERE sr.student_id = ? AND sr.module_id = ?
                ORDER BY sr.main_component_id, sr.sub_component_id
            ";
            $stSR = $conn->prepare($componentsQuery);
            $stSR->bind_param("ii", $student_code, $module_id);
            $stSR->execute();
            $resSR = $stSR->get_result();

            $cGrades = [];
            $cMarks = [];
            while ($cRow = $resSR->fetch_assoc()) {
                // Formatting for display
                $compName = !empty($cRow['as_main_component_name']) ? $cRow['as_main_component_name'] : 'Component';
                if (!empty($cRow['sub_component_name'])) $compName .= " (" . $cRow['sub_component_name'] . ")";

                // Determine display result based on program type
                $dispRes = !empty($cRow['result']) ? $cRow['result'] : 'N/A';
                if (in_array($programme_name, $hd_programs)) {
                    $grade = $cRow['hd_resit3_grade'] ?? $cRow['hd_resit2_grade'] ?? $cRow['hd_resit1_grade'] ?? $cRow['hd_grade'];
                    if ($grade) $dispRes = $grade;
                } elseif ($programme_name == 'Executive Certificate in Management') {
                    $grade = $cRow['ecm_r4_converted_marks_grade'] ?? $cRow['ecm_r3_converted_marks_grade'] ?? $cRow['ecm_r2_converted_marks_grade'] ?? $cRow['ecm_r1_converted_marks_grade'] ?? $cRow['converted_marks_grade'];
                    if ($grade) $dispRes = $grade;
                } elseif ($programme_name == 'Graduate Diploma in Management (Level 6)') {
                    $grade = $cRow['resit_result_4'] ?? $cRow['resit_result_3'] ?? $cRow['resit_result_2'] ?? $cRow['resit_result_1'] ?? $cRow['result'];
                    if ($grade) $dispRes = $grade;
                }

                $component_results_display[] = "<strong>" . $compName . "</strong>: " . $dispRes;

                // Existing HD logic for final grade calculation
                if (in_array($programme_name, $hd_programs)) {
                    $cGrades[] = $cRow['hd_resit3_grade'] ?? $cRow['hd_resit2_grade'] ?? $cRow['hd_resit1_grade'] ?? $cRow['hd_grade'];
                    $cMarks[] = $cRow['hd_resit3_converted_marks'] ?? $cRow['hd_resit2_converted_marks'] ?? $cRow['hd_resit1_converted_marks'] ?? $cRow['hd_converted_marks'] ?? 0;
                }
            }

            if (in_array($programme_name, $hd_programs)) {
                $final_hd_grade = calculateFinalHDGrade($cGrades, $cMarks, $componentCount);
            }
            ...
            $students[] = [
                ...
                'final_hd_grade' => $final_hd_grade,
                'component_results' => implode("<br>", $component_results_display)
            ];
        }
    }
    ...
    if ($report_type === 'student' && !empty($student_id) && !empty($programme_id) && !empty($batch_id)) {
        // Get student details
        $student_query = "SELECT s.student_code, s.first_name, s.last_name, ap.student_registration_id,ap.*,
        ...
        if ($row = $result->fetch_assoc()) {
            $student_details = [
                ...
                'student_name' => $row['first_name'] . ' ' . $row['last_name'],
                'student_registration_id' => $row['student_registration_id'],
                'new_student_registration_id' => $row['new_student_registration_id'],
        ...
        // Get all modules and results for this student
        ...
        $stmt->bind_param("iiiiii", $programme_id, $batch_id, $student_id, $programme_id, $batch_id, $programme_id);
        $stmt->execute();
        $result = $stmt->get_result();
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            // Fetch component results for this module and student
            $componentsQuery = "
                SELECT ac.as_main_component_name, sc.sub_component_name, 
                       sr.*
                FROM student_results sr
            ...
            ";
            $stSR = $conn->prepare($componentsQuery);
            $stSR->bind_param("ii", $student_id, $current_module_id);
            $stSR->execute();
            $resSR = $stSR->get_result();
            ...
            if (in_array($programme_name, $hd_programs)) {
                // Fetch component count for this module
                $compCountQuery = "SELECT COUNT(*) as count FROM allocated_components WHERE module_id = ?";
                $stCC = $conn->prepare($compCountQuery);
                $stCC->bind_param("i", $current_module_id);
                $stCC->execute();
                $resCC = $stCC->get_result();
                $componentCount = 0;
                if ($rowCC = $resCC->fetch_assoc()) {
...
// Function to determine grade based on result
function determineGrade($result)
{
    ...
    // Check if result is a descriptive grade already
    $descriptive_grades = ['distinction', 'merit', 'pass', 'resit', 'withheld', 'absent', 'pending'];
    if (in_array(strtolower($result), $descriptive_grades)) {
        return ucfirst($result);
    }

    // If it's a numeric result
    $result = (float) $result;
    if ($result >= 70) {
        return 'Distinction';
    } elseif ($result >= 60) {
...
{
    $originalMark = $finalMark;
    $ruleApplied = false;

    // ===== Apply 2.6 Rule (Borderline Cases) =====
    if ($finalMark == 49 || $finalMark == 59 || $finalMark == 69) {
        $finalMark = $finalMark + 1; // increase for grade calculation
        $ruleApplied = true;
    }
    ...
    // ===== Show 49+1 Format If Rule Applied =====
    if ($ruleApplied) {
        return $grade . " (" . $originalMark . "+1) (2.6 Rule Applied)";
    }
                ...
                <div class="d-sm-flex align-items-center justify-content-between mb-4 ">
                    ...
                    <?php if ($showResults): ?>
                        <div class="d-flex justify-content-end fixed-header">
                            <button onclick="window.print();" class="btn btn-sm btn-primary shadow no-print mr-2">
                                <i class="fas fa-print fa-sm text-white-50"></i> Print Report
                            </button>
                            <button onclick="exportTableData();" class="btn btn-sm btn-success shadow no-print">
                            ...
                            <div class="card">
                                <div class="card-body">
                                    <div class="row">
                                            ...
                                            <div class="form-group mb-3">
                                                <div class="form-check form-check-inline">
                                                    <input class="form-check-input" type="radio" name="report_type"
                                                        id="report_type_assessment" value="assessment" checked>
                                                    <label class="form-check-label" for="report_type_assessment">By
                                                        Assessment</label>
                                                </div>
                                                <div class="form-check form-check-inline">
                                                    <input class="form-check-input" type="radio" name="report_type"
                                                        id="report_type_student" value="student">
                                                    <label class="form-check-label" for="report_type_student">By
                                                        Student</label>
                                                </div>
                                            </div>
                                        </div>

                                        <div class="col-md-3" id="module_label_row">
                                            <label for="module">Module</label>
                                        </div>
                                        <div class="col-md-9" id="module_input_row">
                                        ...
                                        <div class="col-md-3" id="student_label_row">
                                            <label for="student">Student</label>
                                        </div>
                                        <div class="col-md-9" id="student_input_row">
                                        ...
                                        <div class="col-md-9">
                                            <div class="form-group mb-3">
                                                <input type="text" name="year_display" id="year_display"
                                        ...
                                        <div class="col-md-9">
                                            <div class="form-group mb-3">
                                                <input type="text" name="semester_display" id="semester_display"
                                                    class="form-control" readonly>
                        ...
                        <div class="card-body">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped table-hover" width="100%" cellspacing="0">
                                        ...
                                        <tr>
                                            <th>Date</th>
                                            <td>:
                                                <?php echo !empty($assessment_date) ? date('Y-m-d h:i A', strtotime($assessment_date)) : ''; ?>
                                            </td>
                                        </tr>
                                        <!-- <tr>
                                    <th>Description</th>
                                    ...
                                    <table class="table table-striped table-hover" id="resultsTable" width="100%"
                                        ...
                                        <thead>
                                            <tr>
                                                ...
                                                <th>Result</th>
                                                <th>Grade</th>
                                                <th>Components & Results</th>
                                                <?php if (in_array($programme_name, $hd_programs)): ?>
                                                    <th>Final Result Sent</th>
                                                <?php endif; ?>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <?php foreach ($students as $student): ?>
                                                <tr>
                                                    ...
                                                    <td><?php echo $student['component_results']; ?></td>
                                                    <?php if (in_array($programme_name, $hd_programs)): ?>
                                                        <td><?php echo ucfirst($student['final_hd_grade']); ?></td>
                                                    <?php endif; ?>
                                                </tr>
                                            <?php endforeach; ?>
                                        </tbody>
                            ...
                            <?php elseif ($report_type === 'student' && !empty($student_details)): ?>
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-striped table-hover">
                                                <tbody>
                                                    <tr>
                                                        <th>Student Registration ID:</th>
                                                        <td><?php echo $student_details['student_registration_id']; ?> ||
                                                            <?php echo $student_details['new_student_registration_id']; ?>
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        <th>Student Name:</th>
                                                    ...
                                                    <tr>
                                                        <th>Programme:</th>
                                                        <td><?php echo $student_details['program_name'] . ' - ' . $student_details['batch_name']; ?>
                                                        </td>
                                                    </tr>
                                                    <?php if (!empty($student_status)): ?>
                                                        <tr>
                                                            <th>Student Status:</th>
                                                            <td><?php echo ucfirst($student_status); ?> Student</td>
                                                        </tr>
                                                    <?php endif; ?>
                                                    <tr>
                                                        <th>Year:</th>
                                ...
                                <?php if (!empty($student_modules)): ?>
                                    <div class="table-responsive mt-4">
                                        <table class="table table-striped table-hover" id="studentModulesTable" width="100%"
                                            cellspacing="0">
                                            <thead>
                                                <tr>
                                                    <th>Module Code</th>
                                            ...
                                            <tbody>
                                                <?php foreach ($student_modules as $module): ?>
                                                    <tr>
                                                        ...
                                                        <td><?php echo !empty($module['assessment_date']) ? date('Y-m-d', strtotime($module['assessment_date'])) : 'N/A'; ?>
                                                        </td>
                                                        <td><?php echo $module['grade']; ?></td>
                                                        <td><?php echo $module['component_results']; ?></td>
                                                        <?php if (in_array($programme_name, $hd_programs)): ?>
                                                            <td><?php echo ucfirst($module['final_hd_grade']); ?></td>
                                                        <?php endif; ?>
                                                    </tr>
                                                <?php endforeach; ?>
                                            </tbody>
    ...
    $(document).ready(function() {
        ...
        $('input[name="report_type"]').change(function() {
            ...
            if (reportType === 'assessment') {
                ...
                $('#module_label_row').show();
                $('#module_input_row').show();
                $('#student_label_row').hide();
                $('#student_input_row').hide();
                ...
                $('#module_label_row').hide();
                $('#module_input_row').hide();
                $('#student_label_row').show();
                $('#student_input_row').show();
        ...
        // Default state on page load: show/hide Module vs Student rows
        // to match whichever "Report Type" radio is checked by default
        // (By Assessment / Module is checked by default in the HTML).
        $('input[name="report_type"]:checked').trigger('change');
