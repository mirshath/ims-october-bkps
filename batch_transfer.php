                        ...
                        <div class="col-md-6">
                            <div class="card">
                                ...
                                <div class="card-body">
                                    ...
                                    <!-- New BMS Registration Code Auto Gen -->
                                    <?php
                                    // Get the MAX (numeric) new_student_registration_id from allocate_programme
                                    // Values are plain numbers e.g. 26000630, 26000631, 26000632, 26000633, 26000618
                                    // so we must sort numerically, not as text, or e.g. "26000633" could lose to "26000618"
                                    // due to string comparison edge-cases (different lengths, leading zeros, etc.)
                                    $last_new_reg_id = '';
                                    $lastRegQuery = "SELECT new_student_registration_id 
                                                      ...
                                                      ORDER BY CAST(new_student_registration_id AS UNSIGNED) DESC 
                                                      LIMIT 1";
                                    if ($lastRegResult = $conn->query($lastRegQuery)) {
                                        if ($lastRegRow = $lastRegResult->fetch_assoc()) {
                                            $last_new_reg_id = $lastRegRow['new_student_registration_id'];
                                        }
                                    }

                                    // Work out Last BMS NEW ID + 1, keeping any text prefix and zero-padding intact
                                    // e.g. "BMS00045" -> "BMS00046", "45" -> "46"
                                    $next_new_reg_id = '';
                                    if ($last_new_reg_id !== '') {
                                        if (preg_match('/^(\D*)(\d+)$/', $last_new_reg_id, $m)) {
                                            $prefix        = $m[1];
                                            $numeric_part  = $m[2];
                                            $incremented   = (string)((int)$numeric_part + 1);
                                            $padded        = str_pad($incremented, strlen($numeric_part), '0', STR_PAD_LEFT);
                                            $next_new_reg_id = $prefix . $padded;
                                        } else {
                                            // No numeric part found at the end, can't auto-increment
                                            $next_new_reg_id = $last_new_reg_id;
                                        }
                                    }
                                    ?>
                                    <div class="form-group row">
                                        <label for="last_bms_registration_code" class="col-md-4 col-form-label">Last BMS NEW ID :</label>
                                        <div class="col-md-8">
                                            <input type="text"
                                                id="last_bms_registration_code"
                                                class="form-control"
                                                value="<?php echo htmlspecialchars($last_new_reg_id); ?>"
                                                readonly
                                                tabindex="-1">
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-8">
                                            <input type="text"
                                                ...
                                                value="<?php echo htmlspecialchars($next_new_reg_id); ?>"
                                                data-next-reg-id="<?php echo htmlspecialchars($next_new_reg_id); ?>"
                                                readonly
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            if (!response.error) {
                                ...
                                // Keep the auto-incremented (Last BMS NEW ID + 1) suggestion
                                // unless fetch_student_data.php actually returns a value for this student.
                                if (response.new_student_registration_id) {
                                    $('#bms_registration_code').val(response.new_student_registration_id);
                                } else {
                                    $('#bms_registration_code').val($('#bms_registration_code').data('next-reg-id'));
                                }
                                $('#bms_registration_code').prop('readonly', false);
